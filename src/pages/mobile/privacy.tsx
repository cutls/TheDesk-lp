...
export default function Home() {
	...
	return (
		<Container>
			<Head>
				<title>プライバシーポリシー / Privacy Policy - TheDesk(mobile)</title>
				<meta name="description" content="TheDesk(mobile) - Mastodon client for iOS" />
			...
			<Box>
				...
				<Tabs onChange={(index) => change(index === 0 ? 'ja' : 'en')} index={isJa ? 0 : 1}>
					...
					<TabPanels>
						<TabPanel>
							...
							<Text my={2} ml={5}>
								本アプリは、プッシュ通知を提供するために、開発者のサーバーにアクセスする場合があります。
							</Text>
							<Text my={2} ml={5}>
								この処理は、プッシュ通知の内容を中継するためにのみ行われます。開発者のサーバーにMastodonのアクセストークンを送信し、そのトークンを用いてサーバーにプッシュ通知の購読を行います。購読後速やかにアクセストークンは破棄され、開発者のサーバーは利用者のMastodonアカウントに関するデータや、アクセストークンを一切保存いたしません。また、プッシュ通知の内容はサーバーで復号されAppleの通知配信ネットワークを経由してデバイスに送信されますが、開発者のサーバーはプッシュ通知の内容を保存することはありません。
							</Text>
							<Text my={2} fontSize={18} fontWeight="bold">
								2. サービス提供の基盤について
							</Text>
						...
						<TabPanel>
							...
							<Text my={2}>Effective Date: February 27, 2026</Text>
							<Text my={2}>
							...
							</Text><Text my={2} ml={5} fontWeight="bold">
								(4) Push Notifications
							</Text>
							<Text my={2} ml={5}>
								This App may access the Developer's servers to provide push notifications.
							</Text>
							<Text my={2} ml={5}>
								This process is conducted solely to relay the content of push notifications. The Developer's servers receive your Mastodon access token and use it to subscribe to push notifications on your behalf. The access token is immediately discarded after subscription, and the Developer's servers do not store any data related to your Mastodon account or access token. Additionally, the content of push notifications is decrypted on the server and transmitted via Apple's notification delivery network to your device, but the Developer's servers do not store any content of push notifications.
							</Text>
							<Text my={2} fontSize={18} fontWeight="bold">
				...
				<Text textAlign="center">(c) 2026 TheDesk</Text>
			</Box>
		</Container>
	)
}
